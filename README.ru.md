...
Всё остальное ставит `install.sh` сам: `hyprlock`, `hypridle`,
`brightnessctl`, `librsvg` (рендер SVG-иконок), три Python-библиотеки для
генератора обоев и шрифт JetBrainsMono Nerd Font, из которого берутся глифы.

Ставится только то, что тема действительно использует. `grim` **не**
устанавливается: он нужен лишь тому, кто захочет прислать скриншот, и это
твоё решение, а не установщика.
...
и никогда не переписывает молча то, что ты правил.

Для `colors.conf`, `hypridle.conf` и каждого скрипта из `scripts/` он сравнивает
твою установленную копию с версией из проекта. Совпадают — идёт дальше.
Отличаются (значит, ты что-то менял) — скажет об этом, спросит, переписывать ли,
и только с твоего согласия запишет, сохранив твою версию рядом как
`<имя>.bak-<дата>`. `avatar.png` не трогает никогда.

> Раньше скрипты мониторинга копировались безусловно — без бэкапа и без вопроса.
> Если ты подогнал их под своё железо, обновление молча это стирало. Сейчас
> исправлено, но именно поэтому первое, что делает этот проект, — резервные копии.
...
Спросит подтверждение, а затем — **до** любого удаления — скопирует все файлы,
которые собирается снести, в `~/.config/hypr/monochrome-vivid-removed-<дата>/`.
Снимок делается с текущего состояния, поэтому любые твои правки, сделанные уже
после установки, переживут удаление, даже если исходной резервной копии
`install.sh` давно нет.

Дальше уберёт файлы темы, вычистит добавленную строку `exec-once = hypridle` из
`hyprland.conf` (сделав бэкап самого файла) и предложит восстановить твои
доустановочные конфиги из `~/.config/hypr/monochrome-vivid-backup-*`, если такая
копия найдётся. Системные пакеты (`hyprlock`, `hypridle`, шрифт) uninstall.sh не
трогает — они могут использоваться и без этой темы.
